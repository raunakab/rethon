...
1. **`s1_segmenter`** — grapheme segmentation. Walks the source string and groups characters into raw chunks: contiguous alphabetic runs, numeric runs, individual punctuation characters, whitespace, and quoted strings.

2. **`s2_clusterer`** — token clustering. Consumes the s1 stream and resolves multi-character operators (`:=`, `->`, `|>>`, …), keywords (`fn`, `if`, `return`, …), formatted strings (`f"…"`), and floats (`1.5`).
...
| Variant | Meaning |
|---|---|
| `Normal(LexType)` | A keyword, identifier, operator, number, or string |
...
| `UnknownItem(String)` | Unrecognised character (e.g. `'`, `` ` ``, control characters) |
