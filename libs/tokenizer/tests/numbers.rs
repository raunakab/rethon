...
#[rstest::rstest]
// Plain integer
#[case("42", Ok(vec![T(TokenType::Number("42"))]))]
// Zero
#[case("0", Ok(vec![T(TokenType::Number("0"))]))]
// Multi-digit integer
#[case("1234", Ok(vec![T(TokenType::Number("1234"))]))]
// Float with fractional part
#[case(
    "3.14",
    Ok(vec![T(TokenType::Float("3", Some("14")))])
)]
// Float without fractional part (trailing dot)
#[case(
    "12.",
    Ok(vec![T(TokenType::Float("12", None))])
)]
// Float with leading zero
#[case(
    "0.5",
    Ok(vec![T(TokenType::Float("0", Some("5")))])
)]
// Float zero dot zero
#[case(
    "0.0",
    Ok(vec![T(TokenType::Float("0", Some("0")))])
)]
